...
#GLOBAL VARIABLES
op_status = -1	#-1 means stop sending data, value > 0 need to send data. 2 means chop, 3 means stir
prev_score = 0
curr_score = 0
...
for i in range(COUNTER_THRESH):	#initialize memory for data array (Gz, Gx used for stirring)
        Gz_arr.append(i)
        Gx_arr.append(i)

while True:	#continuously loop, even if don't need to collect data
	while op_status > 0:        #runs when op_status != -1. Perform operation set by op_status
		...
		if op_status == 3:	#stir
			...
			if (counter == COUNTER_THRESH):
					...
					if (max_Gx - min_Gx) < TOP_STIR_SPEED_THRESH and (max_Gx - min_Gx) > BOT_STIR_SPEED_THRESH and (max_Gz - min_Gz) < TOP_STIR_SPEED_THRESH and (max_Gz - min_Gz) > BOT_STIR_SPEED_THRESH:     
						#print('Good speed')
						curr_score = 3
					elif (max_Gx - min_Gx) >= TOP_SPEED_THRESH or (max_Gz - min_Gz) >= TOP_SPEED_THRESH:    #tell player to slow down, return value of 2 for CPU to let player know to slow down
						#print('slow down: 1')
						curr_score = 2
					elif (max_Gx - min_Gx) <= BOT_SPEED_THRESH or (max_Gz - min_Gz) <= BOT_SPEED_THRESH:    #tell player to speed up, return value of 1 for CPU to let player know to speed up
						#print('speed up: 1')
						curr_score = 1
				else:
					curr_score = 0
		...
		if prev_score != curr_score:	#when see a new score, send update to CPU
			...
			message = str(op_status) + str(curr_score)	#format: "23" means chopping (2) got a return score of 3
			client.publish(’1Team8A’, message, qos=1)
