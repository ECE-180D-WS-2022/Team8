...
def on_message(client, userdata, message):
    ...
    if (str(flag_received) == str(FLAG_STIR)):
        ...
        f.close()
        speech_said = True
    elif str(message.topic) == (str(flag_player)+'TeamTrackB'):
        print("Receiving Tracking")
        x_pos = int(message.payload)
...
def main():
    ...
    client.subscribe(str(flag_player)+'Team8TrackA',qos=1)
    #####################
    #WAITING FOR OPPONENT
    #####################
